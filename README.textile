...
Camel's pluggable data formats offer support for pojo/xml marshalling using JAXB, XStream, Castor and XMLBeans. You can use these data formats in your route to sent and receive pojo's to and from web services.
...
With the XML configuration in-place you can now use Camel's DSL to define what web service request are handled by your endpoint:
...
<code>
from("springws:uri:http://example.com/foobar?endpointMapping=#endpointMapping")
.convertBodyTo(String.class).to(mock:example)
</code>
...
For every endpoint with mapping-type @beanname@ one bean of type @org.apache.camel.component.spring.ws.bean.CamelEndpointDispatcher@ with a corresponding name is required in the Registry/ApplicationContext. This bean acts as a bridge between the Camel endpoint and an existing "endpoint mapping":http://static.springsource.org/spring-ws/sites/1.5/reference/html/server.html#server-endpoint-mapping. Example of a route using @beanname@:
